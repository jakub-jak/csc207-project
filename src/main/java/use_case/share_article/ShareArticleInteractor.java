...
public class ShareArticleInteractor implements ShareArticleInputBoundary {
    ...
    public void execute(ShareArticleInputData shareArticleInputData) throws Exception {
        ...
        final String emailBody = createEmailBody(article);
        shareArticleEmailDataAccessObject.sendMail(subject, emailBody, email);
    }

    @Override
    public void executeToOtherEmail(ShareArticleInputData2 shareArticleInputData2) throws Exception {
        final Article article = shareArticleInputData2.getArticle();
        final String email = shareArticleInputData2.getEmail();
        final String subject = article.getTitle();
        final String emailBody = createEmailBody(article);
        shareArticleEmailDataAccessObject.sendMail(subject, emailBody, email);
    }

    private String createEmailBody(Article article) {
        return """
            <!DOCTYPE html>
            <html>
            <head>
